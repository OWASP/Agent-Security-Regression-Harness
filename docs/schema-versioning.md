...
Each schema carries a top-level integer `"version"` field. As of the
introduction of this policy, both schemas are at `"version": 1`:

- `schemas/scenario.schema.json` — `"version": 1`
- `schemas/result.schema.json` — `"version": 1`
...
and an entry in `CHANGELOG.md`. The first time a `"version"` field
is introduced on a schema, that change is itself a PATCH per
[SemVer mapping](#semver-mapping) (it adds a new optional field), and
the introduction is documented in the CHANGELOG so consumers can
start reading it.
...
When a tagged release ships one or more schema changes, the relevant
CHANGELOG entries are carried forward into the release notes;
`CHANGELOG.md` is the canonical record. The release workflow re-runs
the schema validation tests on the tagged commit, so a tag cannot
ship if the schemas drift from the Python validator. See
